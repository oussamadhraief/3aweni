...
export default function EditUserFundraiser() {
	...
	const [UploadingSecondaryImages, setUploadingSecondaryImages] = useState<boolean>(false)
	...
	const handleUpdateFundraiserSecondaryImages = async (publicId: string) => {
		...
		await axios.patch(`/api/fundraiser/secondary-images/${Fundraiser._id}`,{
	...
	const handleUpdateFundraiserSecondaryVideos = async (publicId: string) => {
		...
		await axios.patch(`/api/fundraiser/secondary-videos/${Fundraiser._id}`,{
				...
				<fieldset className="grid grid-cols-4 gap-6 p-6 rounded-md shadow-sm">
					...
					<form onSubmit={handleSubmit} className="grid grid-cols-6 gap-4 col-span-full lg:col-span-3">
						...
						<div className="col-span-full sm:col-span-3">
							...
							<input id="title" name="title" type="text" placeholder="Titre" value={Fundraiser.title} onChange={handleChange} className="w-full rounded-md outline-none text-sm h-9 px-1" />
						</div>
						<div className="col-span-full sm:col-span-3">
				...
				<fieldset className="grid grid-cols-4 gap-6 p-6 rounded-md shadow-sm ">
					...
					<div className='col-span-full grid grid-cols-6 gap-4'>
						...
						<div className='col-span-full'>
							...
							{Loading ?
							<>
							<div className='w-40 h-40 bg-gray-300 animate-pulse'>
							...
							<div className='w-40 h-40 bg-gray-300 animate-pulse'>
								...
								<div className='relative max-w-[288px] h-40 rounded overflow-hidden flex items-center justify-center'>
									<img className="object-cover object-center max-w-full max-h-full self-center" src={`https://res.cloudinary.com/dhwfr0ywo/image/upload/${item}`} alt="content" />
								</div>)}
								</>
							}
